 ...
 */
import { callChat, callProvider, type ChatInput, type ParseInput, type ParseOutput } from '../../src/lib/receiptAi.ts'
...
export function createAiParser(opts: { apiKey?: string; baseUrl?: string; model?: string; visionModel?: string; fetchFn?: typeof fetch }): AiParser {
  ...
  if (!apiKey) return { enabled: false, parse: async () => ({ items: [], total: null, date: null, currency: null, extras: [], merchant: null }), chat: async () => '' }
  return {
    enabled: true,
    parse: (input) => callProvider({ format: 'openai', baseUrl, model: input.image ? visionModel : model, apiKey }, input, { fetchFn }),
    chat: (input) => callChat({ format: 'openai', baseUrl, model: input.image ? visionModel : model, apiKey }, input, { fetchFn }),
  }
}
